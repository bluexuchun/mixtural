...
import Vue from 'vue'
var treeId = -1,
    Tree = function(uid, text, avatar, position, company, type, parentId, width, height, color, bgcolor, treeData) {
        console.log(avatar);
        this.uid = uid;
        this.tree_id = treeId += 1;
        this.parentId = parentId || -1;
        this.text = text;
        this.nickname = text;
        this.position = position;
        this.company = company;
        this.width = width || TREE.config.width;
        this.height = height || TREE.config.height;
        this.color = color || TREE.config.color;
        this.bgcolor = bgcolor || TREE.config.bgcolor;
...
let TREE = {
    ...
    init(uid, text, avatar, position, company, type) {
        treeId = -1;
        return new Tree(uid, text, avatar, position, company, type);
    },
    create(uid, text, avatar, position, company, type) {
        return new Tree(uid, text, avatar, position, company, type);
    },
    /**
    ...
    destroy(tree) {
        ...
        for (var i = 0; i < children.length; i++) {
            if (children[i].tree_id === tree.tree_id) {
                children.splice(i, 1);
                break;
            }
        }
    ...
    async draw(context, tree) {
        ...
        var config = {
            ...
            secondWalk = function(tree, level, X, Y) {
                ...
                if (tree.numChildren()) secondWalk(tree.getFirstChild(), level + 1, X + tree.modifier, Y + maxLevelHeight[level] + config.levelSeparation);
            ...
            positionTree = function(tree) {
                ...
                secondWalk(tree, 0, 0, 0);
                console.log(rootXOffset, rootYOffset);
            },
            ...
            drawNode = async function(tree) {
                ...
                if (tree.drawType == 'avatar') {
                    //画头像
                    let avatar = await Utils.getImageInfo(tree.avatar);
                    context.save();
                    context.beginPath();
                    context.lineWidth = 3;
                    console.log('55555555555555555555555', tree.type);
                    if (tree.type == 1) {
                        context.strokeStyle = "#f9000c";
                    } else if (tree.type == 2) {
                        context.strokeStyle = "#f6c874";
                    } else {
                        context.strokeStyle = "#1e97d0";
                    }
                    context.arc(x + width / 2, y + height / 2, 20, 0, Math.PI * 2);
                    context.stroke();
                    context.closePath();
                    context.clip();
                    context.drawImage(avatar.path, x, y, 40, 40);
                    context.restore();
                    context.draw(true);
                    context.lineWidth = 1;
                    if (tree.drawNickname == 1) {
                        context.strokeStyle = tree.color;
                        context.textBaseline = 'middle';
                        context.strokeText(text, (x + width / 2) - textWidth / 2, y - 10, width);
                        context.strokeStyle = "black";
                    }
                }
                //画头像end
                if (tree.drawType == 'text') {
                    context.save();
                    context.beginPath();
                    context.lineWidth = 3;
                    if (tree.type == 1) {
                ...
                for (var i = 0; tree.numChildren() > 0 && i < tree.numChildren(); i++) {
                    await drawNode(tree.getChildAt(i));
                }
                context.draw(true);
            };
        positionTree(tree);
