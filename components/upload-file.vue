<template>
    <div :id="'file_' + componentId">
        <div class="files">
            <div class="files-sort" :style="{height: area_height + 'rpx'}" id="drag">
                <div v-for="(item, index) in currentList" :key="index" :data-index="index" class="files-sort-item" :class="{'active': current === index}" :style="{left:item.x + 'rpx', top:item.y + 'rpx'}" @longpress.stop="touchstart" @tap.stop="tap" @touchmove.stop="touchmove" @touchend.stop="touchend">
                    <img mode="aspectFill" class="files-sort-item-file" src="../static/images/file.png">
                </div>
                <div class="files-sort-item add" :style="{left:addX + 'rpx', top:addY + 'rpx'}" @click="addFiles">
                    <div>
                        <i class="iconfont icon-file"></i> 添加文件
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        async addFiles() {
            ...
            if (_this.currentList.length >= _this.number) {
                Utils.toast('最多上传' + _this.number + '个文件');
                return;
            }
            const tempFilePaths = await Utils.chooseFile();
            ...
            for (let key in pictures) {
                let result = await Utils.uploader(pictures[key].url, 'file');
                console.log(result);
                if (result.status == 1) {
                    _this.lists.push({
