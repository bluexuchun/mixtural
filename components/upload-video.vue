<template>
    <div :id="'video_' + componentId">
        <div class="videos">
            <div class="videos-sort" :style="{height: area_height + 'rpx'}" id="drag">
                <div v-for="(item, index) in currentList" :key="index" :data-index="index" class="videos-sort-item" :class="{'active': current === index}" :style="{left:item.x + 'rpx', top:item.y + 'rpx'}" @longpress.stop="touchstart" @tap.stop="tap" @touchmove.stop="touchmove" @touchend.stop="touchend" @click="prevideVideo(item.video)">
                    <img mode="aspectFill" class="videos-sort-item-video" :src="item.video + '?x-oss-process=video/snapshot,t_0,f_jpg,w_0,h_0,m_fast'">
                </div>
                <div class="videos-sort-item add" :style="{left:addX + 'rpx', top:addY + 'rpx'}" @click="addVideos">
                    <div>
                        <i class="iconfont icon-video_add"></i> 添加视频
                    </div>
                </div>
            </div>
        </div>
        ...
        <video id="myVideo" class="video-preview" @fullscreenchange="fullScreen" :src="video_url"></video>
    </div>
</template>
<script>
    ...
    methods: {
        ...
        async addVideos() {
            ...
            if (_this.currentList.length >= _this.number) {
                Utils.toast('最多上传' + _this.number + '个视频');
                return;
            }
            const tempFilePath = await Utils.chooseVideo();
            Utils.loading();
            let result = await Utils.uploader(tempFilePath, 'video');
            console.log(result);
            if (result.status == 1) {
                _this.lists.push({
